...
""")
        ...
        with saliweb.test.working_directory(j.directory):
            ...
            for err in ('Traceback (most recent call last):',
                ...
                with open('test.o1234.1', 'w') as fh:
                    fh.write(err)
                self.assertRaises(allosmod.AllosModLogError,
                                  j.check_log_errors)
            # Tracebacks handled internally by AllosMod aren't considered
            # errors
            with open('test.o1234.1', 'w') as fh:
    ...
    def test_check_preprocess(self):
        ...
        j = self.make_test_job(allosmod.Job, 'RUNNING')
        with saliweb.test.working_directory(j.directory):
            for f in ('dirlist', 'dirlist_all', 'jobcounter', 'job-state',
                      'pwout', 'sge-script.sh', 'sge-script.sh.o9058604.1'):
                open(f, 'w').close()
            os.mkdir('input')
